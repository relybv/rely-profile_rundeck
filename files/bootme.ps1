...
# This script installs puppet 4.x and deploy the manifest using puppet apply -e "include profile_rundeck"
# Usage:
# <powershell>
# Set-ExecutionPolicy Unrestricted -Force
# icm $executioncontext.InvokeCommand.NewScriptBlock((New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/relybv/rely-profile_rundeck/master/files/bootme.ps1')) -ArgumentList ("profile_rundeck")
#</powershell>


  param(
    [string]$role = "profile_rundeck"
  )

  $puppet_source = "https://github.com/relybv/rely-profile_rundeck.git"
  ...
  if(!(Test-Path -Path "C:\ProgramData\PuppetLabs\code\modules\profile_rundeck" )){
    $clone_args = @("clone",$puppet_source,"C:\ProgramData\PuppetLabs\code\modules\profile_rundeck" )
  } else {
    $clone_args = @("pull")
    Set-Location -Path "C:\ProgramData\PuppetLabs\code\modules\profile_rundeck"
  }
